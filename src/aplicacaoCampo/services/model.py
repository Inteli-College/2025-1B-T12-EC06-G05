...
def run_detection_model(model_path, image_folder):
    ...
    for img_path in images:
        ...
        results = model(img)[0]
        result_img_path = os.path.join(results_folder, f"detect_{os.path.basename(img_path)}")
    ...
    with open(crack_info_path, 'w') as f:
        json.dump({'images_with_cracks': images_with_cracks, 'crack_counts': crack_counts}, f)
